<template>
    <view class="">
        <custom fixed="fixed" background="#ffffff" mode="2" title="编辑商品" :titleBold="false"></custom>
...
export default {
    ...
    methods: {
        getMinSku() {
            let minPrice = this.goodInfo.liveStoreGoodsSkus[0].price
            this.goodInfo.liveStoreGoodsSkus.forEach(item => {
                if(item.price < minPrice) {
                    minPrice = item.price
                }
            })
            return this.goodInfo.liveStoreGoodsSkus.find(item => item.price === minPrice) || null
        },
        getImg(imgs) {
            return imgs?.split(',')[0] || ''
        },
